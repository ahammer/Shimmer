...
class ClaudeAdapterTest {
    ...
    @BeforeEach
    fun setup() {
        adapter = ClaudeAdapter(client = mockk(relaxed = true))
    }
    ...
    @Test
    fun `buildSystemPrompt returns base instructions for String result`() {
        val result = adapter.buildSystemPrompt(defaultContext, String::class)
        assertEquals("test", result)
    }

    @Test
    fun `buildSystemPrompt appends JSON schema for data class result`() {
        val result = adapter.buildSystemPrompt(defaultContext, SimpleResult::class)
        assertTrue(result.startsWith("test"), "Should start with base instructions")
        assertTrue(result.contains("schema"), "Should contain schema instruction")
        assertTrue(result.contains("value"), "Should contain schema field")
